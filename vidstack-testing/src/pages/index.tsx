...
import { MediaCaptions, MediaFullscreenButton, MediaMuteButton, MediaOutlet, MediaPlayButton, MediaPlayer, MediaPoster, MediaSliderThumbnail, MediaSliderValue, MediaTime, MediaTimeSlider, MediaTooltip, MediaVolumeSlider } from "@vidstack/react";
...
import { type Anime, type EpisodeData, type Source } from "~/types";
import { SettingsPanel } from "~/components/settingsPanel";
import { ChaptersPanel } from "~/components/chaptersPanel";

const Watch: NextPage<Props> = ({ episodeNumber, episodeSelector, episodes, media, sources, subtitles, thumbnails }) => {
    ...
    function updateCueMt(value?: string) {
        //
    }

    function updateCueMb(value?: string) {
        //
    }

    function addCues() {
        const introStart = sources.intro.start ?? 0;
        const introEnd = sources.intro.end ?? 0;
        ...
        const videoDuration = playerRef.current?.$store.duration();

        const cues = [
            { startTime: 0, endTime: introStart, text: "" },
            { startTime: introStart, endTime: introEnd, text: "Intro" },
            { startTime: introEnd, endTime: outroStart, text: "" },
            { startTime: outroStart, endTime: outroEnd, text: "Outro" },
            { startTime: outroEnd, endTime: videoDuration, text: "" }
        ];

        console.log(cues)

        for (const cue of cues) {
            const vttCue = new VTTCue(typeof(cue.startTime) === "number" ? cue.startTime : 0, typeof(cue.endTime) === "number" ? cue.endTime : 0, cue.text);
            track.addCue(vttCue);
        }
    ...
    return (
        <>
            <main className="grid md:grid-cols-1 lg:grid-cols-[72.5%,1fr] 2xl:gap-2" id="player-container">
                <MediaPlayer
                    ref={playerRef}
                    title={media.title.english ?? media.title.romaji ?? media.title.native ?? ""}
                    className="aspect-video h-full !rounded-b-none"
                ...
                >
                    <MediaOutlet className="relative">
                        ...
                        <MediaPoster alt={(media.title.english ?? media.title.romaji ?? media.title.native ?? "") + " Poster"} />
                    </MediaOutlet>
                        ...
                        <div className={`${styles.mediaControlsGroup ?? ""} flex flex-col justify-center items-center text-2xl w-full`}>
                            <div className="flex flex-row justify-between items-center text-white w-full">
                                <div className={`flex flex-row justify-start items-center ${styles.ui ?? ""}`}>
                                    <div className="w-[40px] h-[40px] mt-[5px] ml-[10px] cursor-pointer stroke-white">
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    const subDub: string | string[] | undefined = context?.params?.subDub ?? "sub";
    ...
    const episodeCovers = await (await axios.get(`${String(env.BACKEND_URL)}/episode-covers?id=${String(id)}&apikey=${String(env.API_KEY)}`)).data as { episode: number, img: string }[];
    ...
    for (let i = 0; i < content.length; i++) {
        ...
        for (let j = 0; j < (providerEpisodes?.episodes ?? []).length; j++) {
            ...
            if (providerEpisodes?.providerId.toLowerCase() === (provider).toLowerCase()) {
                ...
                if (subDub === "dub") {
                    if (episode?.hasDub) {
                        episodeSelector.push({
                            ...
                            url: `/watch/${id as string}/${provider}/${encodeURIComponent(episode.id)}/dub`,
                    ...
                    episodeSelector.push({
                        ...
                        number: episode?.number ?? i + 1,
                        length: capitalize(providerEpisodes.providerId),
                        url: `/watch/${id as string}/${provider}/${encodeURIComponent(episode?.id ?? "")}/sub`,
                        selected: episode?.id === watchId
                    })
                }
            }
    ...
    for (let i = 0; i < sources.sources.length; i++) {
        if (sources.headers && Object.keys(sources.headers).length > 0) {
            Object.assign(sources.sources[i] ?? {}, { url: `${String(process.env.M3U8_PROXY)}/proxy/m3u8/${encodeURIComponent(String(sources.sources[i]?.url))}/${encodeURIComponent(JSON.stringify(sources.headers))}` })
        }
    }
